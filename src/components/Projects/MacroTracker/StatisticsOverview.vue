...
} from '@/Logic/MacroTracker/statisticsFunctions'
  ...
  <section class="card mb-2">
    ...
    <header class="card-header p-3 d-flex">
      <div class="d-flex flex-column">
        <h3 class="m-2">
          Statistics
          <h6>Total, average and nutrient stats for each meal</h6>
        </h3>
      ...
      <template v-else>
        <template v-if="StatsToShow">
          ...
          <div class="mt-2" v-for="(date, index) in Object.keys(stats_for_dates)" :key="index">
            ...
            <div
              class="m-4 ms-0"
              v-for="(meal_name, index) in Object.keys(stats_for_dates[date].meals)"
              :key="index"
            >
