...
<template>
  <template v-for="mealModal in mealModalInformation" :key="mealModal.formulate_type">
    <FormulateMeal
      :formulate_type="mealModal.formulate_type"
      :meal="mealModal.meal"
      :random="randomNumber"
    />
  </template>
  ...
  <section class="card">
    <div class="card-header">
      ...
      <div class="d-flex flex-wrap">
        ...
        <button
          class="m-2 create_button btn-success btn btn-lg"
          data-bs-toggle="modal"
          data-bs-target="#create_meal_modal"
          @click="createMeal()"
        >
          Create a meal
        </button>
    ...
    <div class="card-body">
      ...
      <template v-else>
                ...
                <CAccordionBody>
                  <template v-if="meal.ingredients.length > 1">
                    ...
                    <div class="d-flex flex-wrap" id="total_macros">
                      <small class="rounded border border-1 p-2 m-1"
                        >Protein: {{ meal['protein'] }}g</small
                      >
                      <small class="rounded border border-1 p-2 m-1"
                        >Calories: {{ meal['calories'] }}kcal</small
                      >
                      <small class="rounded border border-1 p-2 m-1"
                        >Carbohydrates: {{ meal['carbohydrates'] }}g</small
                      >
                      <small class="rounded border border-1 p-2 m-1">Fat: {{ meal['fat'] }}g</small>
                      <small class="rounded border border-1 p-2 m-1"
                        >Sugar: {{ meal['sugar'] }}g</small
                      >
                    </div>
                  </template>
                  ...
                  <div class="d-flex mt-2 btn-group btn-group-md">
                    ...
                    <button
                      ...
                      data-bs-toggle="modal"
                      data-bs-target="#edit_meal_modal"
                      @click="editMeal(meal)"
                    >
                      Edit
        ...
        <div v-if="list_of_meals[0].length == 0" class="ml-5 mb-2 mt-2">
          ...
          <button
            type="button"
            class="btn-success btn btn-lg ml-2"
            data-bs-toggle="modal"
            data-bs-target="#create_meal_modal"
            @click="hideAlert(), createMeal()"
          >
            <h5>Create a meal</h5>
          </button>
        </div>
