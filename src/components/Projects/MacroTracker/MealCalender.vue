    ...
    <section class="card" id="meals_for_given_date">
        ...
        <div class="card-body">
            <template v-if="zero_meals_for_time_period">
                <h4> There aren't meals in the calender between {{ reverseInputDateFormat(selected_start_date) }} and {{
                    reverseInputDateFormat(selected_end_date) }} </h4>
            </template>
            <template v-else v-for="(date, index) in Object.keys(meals_for_time_of_day)" :key="index">
                ...
                <div v-if="!(meals_for_time_of_day as Meals_for_time_of_day)[date].zero_meals_to_show"
                    class="d-flex flex-wrap">
                    <div v-for="(meals_for_given_time, meal_time) in (meals_for_time_of_day as Meals_for_time_of_day)[date].meal_periods"
                        :key="meal_time">
