...
import type { User_input_field } from '@/Logic/MacroTracker/types'
import { ValidateText } from '@/Logic/MacroTracker/validation'
import { ref } from 'vue'
import { user_validation_arr } from '@/Logic/MacroTracker/initVariables'
...
<template>
  <div class="mt-2" :class="user_input_field.class">
    <div
      v-if="user_input_field.type && user_input_field.type == 'prepend'"
    ...
    <input
      ...
      :type="user_input_field.inputType"
      :placeholder="user_input_field.placeholder"
      :value="user_input_field.value"
      :name="user_input_field.identifier"
      required
    />
    <div
      v-if="user_input_field.type && user_input_field.type == 'append'"
      :class="`input-group-${user_input_field.type}`"
    >
      <span class="input-group-text"> {{ user_input_field.attachment }} </span>
    </div>
