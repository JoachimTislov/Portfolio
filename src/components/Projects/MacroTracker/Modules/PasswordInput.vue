...
<template>
    ...
    <div class="mt-1">
        <label :for="password_details.name" class="form-label"> {{ password_details.label }} Password: </label>
        <input
            @input="change_password_validation[password_details.name] = ValidateText($event, password_validation_message, 'Password', className)"
            :class="className" :name="password_details.name" type="password">
